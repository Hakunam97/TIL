...
class Clock:
    ...
    def tick(self):
        ...
        if self.second.tick():
            if self.minute.tick():
                self.hour.tick()    # Counter 클래스의 tick 메소드는 해당 인스턴스의 value를 1만큼 증가시키고 최댓값에 도달하면 value의 값을 0으로 바꾼 후 True를 리턴
                # Counter 클래스의 tick 메소드임을 나타냄
